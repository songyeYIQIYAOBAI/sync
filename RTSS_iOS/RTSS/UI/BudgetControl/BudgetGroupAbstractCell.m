...
@interface BudgetGroupAbstractCell ()

@property (nonatomic,assign) CGSize availableSize;

@end

@implementation BudgetGroupAbstractCell
...
//    groupHistoryButton = [CommonUtils buttonWithType:UIButtonTypeCustom frame:historyRect title:nil imageNormal:[UIImage imageNamed:@"budget_group_history"] imageHighlighted:nil imageSelected:nil addTarget:self action:@selector(historyButtonClicked:) tag:0];
//    [self.contentView addSubview:groupHistoryButton];
    ...
    groupSeperatorImageView.backgroundColor = [RTSSAppStyle currentAppStyle].separatorColor;
    [self.contentView addSubview:groupSeperatorImageView];
}

#pragma mark button clicked
- (void)historyButtonClicked:(UIButton *)historyButton {
    if (delegate && [delegate respondsToSelector:@selector(budgetGroupAbstractCellHistoryButtonDidClicked:)]) {
        [delegate budgetGroupAbstractCellHistoryButtonDidClicked:self];
    }
}
